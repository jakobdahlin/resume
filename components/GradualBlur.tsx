...
'page-header': {
  position: 'top',
  height: '5rem',          // smaller band
  strength: 1.5,           // gentler blur
  divCount: 10,            // smoother gradient
  curve: 'ease-out',       // soft fade near edges
  target: 'page',
...
const getGradientDirection = (position: string): string => {
  const d: Record<string, string> = { top: 'to top', bottom: 'to bottom', left: 'to left', right: 'to right' };
...
const GradualBlur: React.FC<GradualBlurProps> = (props) => {
  ...
  const blurDivs = useMemo(() => {
    ...
    for (let i = 1; i <= config.divCount; i++) {
      ...
      const divStyle: CSSProperties = {
        // tiny background to guarantee backdrop-filter on all engines
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    if (isVertical) {
      ...
      baseStyle.width = isPageTarget ? '100vw' : (responsiveWidth || '100%');
      baseStyle[config.position] = 0;
      baseStyle.left = 0;
      baseStyle.right = 0;
    } else if (isHorizontal) {
      baseStyle.width = isPageTarget ? '100vw' : (responsiveWidth || responsiveHeight);
      baseStyle.height = isPageTarget ? '100vh' : '100%';
      baseStyle[config.position] = 0;
      baseStyle.top = 0;
      baseStyle.bottom = 0;
      baseStyle.left = 0;
